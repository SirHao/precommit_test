...
class Timesheet extends AppModel
{
    ...
    public $no_need_update_effort = false;
    ...
    public function update_effort($data = array())
    {
        $ret = false;
            ...
            if ('story' == $entity_type) {
                ...
                if (!empty($story)) {
                    ...
                    if (TEST_EFFORT_WORKSPACE == $workspace_id) {
                        write_effort_log(
                            'Timesheet::update_effort',
                            $workspace_id,
                            $entity_id,
                            $story['Story']['parent_id'],
                            'story',
     ...
     * @return float
     */
    public function _get_latest_timesheet_remain($entity_type, $entity_id)
    {
        $rand = microtime(true);
    ...
    public function update_timesheets_after_delete($del_timesheet)
    {
        $this->update_timesheets_after_add_timesheet($del_timesheet['Timesheet']);
    }

    public function update_effort_after_delete($del_timesheet)
    {
     ...
     * @return array 责任人数组
     */
    public function getOwners($objecttype, $object_id)
    {
     ...
     * @return array 时间花费数组
     */
    public function get_totalSpents($objecttype, $object_id, $owner = null)
    {
        if (!is_numeric($object_id)) {
            return 0;
        }
     ...
     * @return int 剩余工时
     */
    public function get_recentRemain($objecttype, $object_id, $owner = null)
    {
        if (!is_numeric($object_id)) {
            return 0;
        }
    ...
    public function get_recentTimesheet($objecttype, $object_id, $owner = null)
    {
        $condition = array();
     ...
     * @return int 总花费
     */
    public function get_total_spent_before_date($objecttype, $object_id, $spent_date)
    {
        if (!is_numeric($object_id)) {
            return 0;
        }
    ...
    public function get_total_spent_on_date($objecttype, $object_id, $spent_date)
    {
    ...
    public function get_total_spent_by_spentdate_createdate($objecttype, $object_id, $spent_date, $createdate=null)
    {
        if (!empty($createdate)) {
    ...
    public function get_total_spent_at_spentdate_before_createdate($objecttype, $object_id, $spent_date, $createdate)
    {
        if (!is_numeric($object_id)) {
            return 0;
        }
     ...
     * @return int 当日剩余工时
     */
    public function get_remain_on_date($objecttype, $object_id, $spentdate)
    {
    ...
    * @return int 当日剩余工时
    */
    public function get_remain_before_date($objecttype, $object_id, $spentdate)
    {
     ...
     * @return array 工时花费记录
     */
    public function get_timesheets_with_name($conditions = null, $fields = null, $order = null, $limit = null)
    {
     ...
     * @return boolean 是否成功
     */
    public function move_timesheet_workspace($entity_id, $new_workspace_id)
    {
    ...
    public function save_move_timesheet_workspace($entity_type, $entity_ids, $new_workspace_id)
    {
     ...
     * @author markguo
     */
    public function move_timesheets($target_entity, $source_entity, $modified_fields=array())
    {
        /*
     ...
     */
    public function _need_update_effort($data)
    {
    ...
    public function save_without_update_effort(&$data = null, $validate = true, $fieldList = array())
    {
    ...
    public function append_id_if_exist(&$data)
    {
    ...
    public function is_need_find_exist_timesheet($data)
    {
        if (!empty($data['id'])) {
            return false;
        }
     ...
     */
    public function get_story_effort($story_id, $workspace_id)
    {
        $story_timesheet = $this->findAll(
            array('workspace_id' => $workspace_id, 'entity_id' => $story_id, 'entity_type' => 'story'),
            'timespent,timeremain,exceed',
            'created desc'
        );
        // debug($workspace_id);
        // debug($story_id);
     ...
     */
    public function get_timesheets_by_owner($owners, $date_begin, $date_end, $conditions = array())
    {
        $ret = array();
    ...
    public function resort_owners_by_timesheet_sepnt(&$owners, $date_begin, $date_end, $conditions = array())
    {
        $conditions = $this->deal_conditions($owners, $date_begin, $date_end, $conditions, $is_enabled_task, $is_enabled_story);
    ...
    * @author voladozhang
    */
    public function deal_conditions(&$owners, $date_begin, $date_end, $conditions, &$is_enabled_task, &$is_enabled_story)
    {
        if (!is_array($owners)) {
     ...
     * @param string $date_end 结束时间
     */
    public function _get_time_range($date_begin, $date_end)
    {
        $ret = array();
     ...
     */
    public function get_workitems_by_timesheets(&$timesheets)
    {
        $ret = array();
    ...
    public function get_objects_spent_before($entity_type, $objects, $date)
    {
    ...
    public function get_object_spent_before($entity_type, $id, $date)
    {
        $function = "get_{$entity_type}_spent_before";
        return 	$this->$function($id, $date);
    }

    public function get_task_spent_before($id, $date)
    {
        $spent = 0;
    ...
    public function get_story_spent_before($id, $date)
    {
        $spent = 0;
        $task = new Task();
     ...
     * @author kuncai
     */
    public function get_object_remain_in($entity_type, $id, $effort, $date)
    {
        $condition = array();
    ...
    public function get_object_passtotalspent_in($entity_type, $id, $effort, $date)
    {
        //当天过去所有花费
        $condition = array();
        $condition['entity_type'] = $entity_type;
     ...
     * @return boolean
    */
    public function remove_all_timesheet($entity_type, $entity_id)
    {
    ...
    public function get_timespent_by_iteration($type, $entity_ids, $workspace_id, $iteration_infor)
    {
     ...
     * @author kuncai
     */
    public function get_timespent_timeremains($entity_ids, $workspace_id, $iteration_infor)
    {
        $timesheet_condition = array();
        $timesheet_condition['entity_id'] = $entity_ids;
        $timesheet_condition['workspace_id'] = $workspace_id;
     ...
     * @author kuncai
     */
    public function process_story_tasks_time_spents($story_tasks, $time_spents, $iteration_date)
    {
            ...
            if (!empty($tasks)) {
                ...
                foreach ($workitems[$key]['tasks'] as $task) {
                    foreach ($iteration_date as $date=>$value) {
                        ...
                        $workitems[$key][$model]['spents'][$date]['color'] = $this->get_color(
                            $workitems[$key][$model]['spents'][$date]['totalspent']+$task['Task']['spents'][$date]['timeremain'],
                            $item['effort'],
                            $remain,
                            $pre_timeremain
                        );
     ...
     * @author kuncai
     */
    public function get_color($totaleffort, $effort, $timeremain, $pre_timeremain)
    {
     ...
     * @author kuncai
     */
    public function get_total_spents_remains($story_tasks)
    {
        $total_times = array();
    ...
    public function merge_story_task_for_progress($all_stories, $all_tasks, $workspace)
    {
        $story_tasks = array();
     ...
     */
    public function is_add_timesheet_for_last_date($date, $objtype, $obj_id)
    {
     ...
     * @return bool
     */
    public function change_recent_timesheet_for_object($timeremain, $objtype, $obj_id)
    {
     ...
     * @return boolean
     */
    public function save_timesheet($entity_type, $object_id, $remain_effort, $data, $workspace_id, $is_edit = false)
    {
     ...
     *	@author kuncai
     */
    public function save_timesheet_new($data)
    {
    ...
    public function validate_not_empty($data)
    {
        if (empty($data['entity_id'])) {
            return false;
        }
    ...
    public function update_timesheet($data)
    {
    ...
    public function is_add_timesheet_in_feature($spentdate)
    {
    ...
    public function get_effort_by_entity($entity_type, $entity_id)
    {
    ...
    public function is_have_timespent_by_entity($entity_type, $entity_id)
    {
    ...
    public function update_last_timesheet_after_add_timesheet($add_timesheet, $remain)
    {
    ...
    public function update_timesheets_after_add_timesheet($add_timesheet)
    {
    ...
    public function cal_timeremain($total_spent, $effort)
    {
    ...
    public function find_timesheets_after($timesheet)
    {
        $condition = array();
     ...
     */
    public function cal_exceed($total_spent, $timeremain, $effort)
    {
        return number_format($total_spent + $timeremain - $effort, 2);
    }

    public function _deal_conditions_by_workspace_switches($conditions, &$is_enabled_story, &$is_enabled_task)
    {
    ...
    public function get_exisits_entity_ids($conditions)
    {
