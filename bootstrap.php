 ...
 * @access public
 */
	function html($string, $remove = false, $remove_nl = false) {
		if(is_array($string)) {
			return $string;
		}
	...
	function html_with_br($string, $remove = false, $remove_nl = false) {
...
class MyClean {
	...
	static function cleanValue($value) {
	  ...
	  * @access public
	  */
	static function html($value, $remove = false) {
		GLOBAL $clean;
		return $clean->html($value, $remove);
	}

	static function inline_attr_html($value, $remove = false) {
		GLOBAL $clean;
		return $clean->html($clean->html($value, $remove));
	}

	static function html_with_br($value, $remove = false) {
		GLOBAL $clean;
     ...
     * @return mixed|string
     */
    static function encodeEmoji($str){
    	return $str;
        if (!is_string($str)) {
            return $str;
        }
     ...
     * @return mixed
     */
    static function decodeEmoji($str){
    	return $str;
        $text = json_encode($str); //暴露出unicode
	...
	static function clean_url_xss($from_url) {
	 ...
	 */
	static function cleanXSS(&$mixed, $remove = false) {
		//默认将处理掉参数中的js代码
		if (is_array($mixed)) {
	...
	static function cleanXSSWithKey($mixed, $remove = false) {
		$mix = array();
		//默认将处理掉参数中的js代码
		if (is_array($mixed)) {
			foreach($mixed as $key => $value) {
	 ...
	 */
	static function cleanJUMP($ref_url) {
		if (ISDEV) {
			$jump_scope = array(
					TAPD_DOMAIN,
					'lion.oa.com',
	 ...
	 */
	static function richText($richText) {
	 ...
	 */
	static function cleanUTF8mb4($text) {
	 ...
	 *
	 */
	static function cleanBackspaces($text) {
	 ...
	 * @return string  处理后的文本
	 */
	static function cleanNick($nick) {
	 ...
	 * @return string  处理后的文本
	 */
	static function rich2text($richText) {
	 ...
	 * @author domaintian
	 */
	static function clean_user_name($name_string = '') {
	...
	static function jsString($str, $no_wrap=false) {
		if($no_wrap){
	...
	static function safeCakeLikeCondition($str, $special_escape_list='%_\\'){
		return addcslashes($str, $special_escape_list);
	}

	static function sqlString($str, $special_escape_list='%_') {
	...
	*/
	function calc_relativeTime($time) {
		...
		} else if (date('Y-m-d', $time) == date('Y-m-d', time())){
			//某时间为今天
			return '1. Today';
		} else if (date('Y-m-d', $time) == date('Y-m-d', strtotime('yesterday'))){
			//某时间为昨天
			return '2. Yesterday';
		}
		...
		if ($time > $firstday- 604800){
			//上周
			return '4. Last week';
		}else if ($time >= $firstday - 604800 * 2){
			//两周前
			return '5. Two weeks ago';
		}else if ($time >= $firstday - 604800 * 3){
			//三周前
		...
		}else if ($time >= mktime(0, 0, 0, date('m') - 1, 1, date('Y'))){
			//上月
			return '8. Last month';
		} else if ($time < mktime(0, 0, 0, date('m') - 1, 1, date('Y'))){
			//很早以前
	...
	*/
	function _workitem_appendfield_sort($value1, $value2) {
		if(isset($value1['sort']) && isset($value2['sort']))
			$key = 'sort';
		else if(isset($value1['Sort']) && isset($value2['Sort']))
				$key = 'Sort';
			else
				return 1;
...
function get_date_ch($time, $with_his = true) {
	...
	if($current_d == $time_d && $time_num > time()-24*60*60) {
		if($time_num > time()-5*60) {
			return __('刚刚', true);
		}else if($time_num > time()-10*60) {
			return __('10分钟前', true);
		}else if($time_num > time()-35*60) {
			return __('半小时前', true);
		}else if($time_num > time()-50*60) {
			return __('45分钟前', true);
		}else if($time_num > time()-1.5*60*60) {
			return __('1小时前', true);
		}else{
...
//	}
	...
	function get_menshen_security_suffix(){
		$enable_subffix = defined('ENABLE_MENSHEN_SECURITY_SUFFIX') && ENABLE_MENSHEN_SECURITY_SUFFIX;
		if($enable_subffix) $enable_subffix = defined('MENSHEN_SECURITY_SUFFIX') ? MENSHEN_SECURITY_SUFFIX : '/security';
...
function get_system_type($controller = array()) {
	...
	if(isset($controller->plugin) && isset($controller->params['url']['mini']) && $controller->params['url']['mini']==1){
		$tag .= '_MINI';
	}else if($detect->isWeWorkDesktop()){	//企业微信PC版
		$tag .= '_QYWXPC';
	}else if($detect->isWeWork()){	//企业微信移动端
		$tag .= '_QYWXAPP';
	}else if ( IS_WEIXIN ) {	//公众号
...
function has_out_tapd_query_for_in_data_error() {
	if(!IS_EXTERNAL) {
		return false;
	} else if(IS_EXTERNAL && !IS_CLOUD && isset($GLOBALS['out_tapd_query_for_in_data_error']) && $GLOBALS['out_tapd_query_for_in_data_error']) {
		return true;
	}
	return false;
...
function write_file_with_lock($full_file_name, $content, $timeout=1000){
	if ($fp = fopen($full_file_name, 'a+')) {
		...
		do {
			...
			if (!$can_write) usleep(round( rand(0, 100)*100 ));
